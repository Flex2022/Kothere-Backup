...
class TrialBalanceCustomHandler(models.AbstractModel):
    _name = 'account.partner.trial.balance.report.handler'
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals, warnings=None):
        ...
        lines = [line[1] for line in self.env['partner.ledger.handler.custom']._dynamic_lines_generator(report, options, all_column_groups_expression_totals, warnings=warnings)]
    ...
    def _create_column_group_initial_balance(self, report, options, previous_options, default_group_vals, side_to_append):
        initial_balance_options = self.env['partner.ledger.handler.custom']._get_options_initial_balance(options)
