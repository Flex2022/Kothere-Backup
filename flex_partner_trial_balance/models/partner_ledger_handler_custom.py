...
class TrialBalanceCustomHandler(models.AbstractModel):
    _name = 'partner.ledger.handler.custom'
    _inherit = 'account.report.custom.handler'
    _description = 'Partner Trial Balance Custom Handler'
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals, warnings=None):
        ...
        for partner, column_group_results in self._query_values(report, options):
            eval_dict = {}
            has_lines = False
            for column_group_key, results in column_group_results.items():
                partner_sum = results.get('sum', {})
                partner_un_earn = results.get('unaffected_earnings', {})

                partner_debit = partner_sum.get('debit', 0.0) + partner_un_earn.get('debit', 0.0)
                partner_credit = partner_sum.get('credit', 0.0) + partner_un_earn.get('credit', 0.0)
                partner_balance = partner_sum.get('balance', 0.0) + partner_un_earn.get('balance', 0.0)

                eval_dict[column_group_key] = {
                    'amount_currency': partner_sum.get('amount_currency', 0.0) + partner_un_earn.get('amount_currency', 0.0),
                    'debit': partner_debit,
                    'credit': partner_credit,
                    'balance': partner_balance,
                }

                max_date = partner_sum.get('max_date')
                has_lines = has_lines or (max_date and max_date >= date_from)

                totals_by_column_group[column_group_key]['debit'] += partner_debit
                totals_by_column_group[column_group_key]['credit'] += partner_credit
                totals_by_column_group[column_group_key]['balance'] += partner_balance

            lines.append(self._get_partner_title_line(report, options, partner, has_lines, eval_dict))
    ...
    def _custom_unfold_all_batch_data_generator(self, report, options, lines_to_expand_by_function):
        ...
        has_more_per_partner_id = {}

        unlimited_aml_results_per_partner_id = self._get_aml_values(report, options, partner_ids_to_expand)[0]
        if limit_to_load:
            # Apply the load_more_limit.
            # load_more_limit cannot be passed to the call to _get_aml_values, otherwise it won't be applied per partner but on the whole result.
            # We gain perf from batching, but load every result ; then we need to filter them.

            aml_results_per_partner_id = {}
            for partner_id, partner_aml_results in unlimited_aml_results_per_partner_id.items():
                partner_values = {}
                for key, value in partner_aml_results.items():
                    if len(partner_values) == limit_to_load:
                        has_more_per_partner_id[partner_id] = True
                        break
                    partner_values[key] = value
                aml_results_per_partner_id[partner_id] = partner_values
        else:
            aml_results_per_partner_id = unlimited_aml_results_per_partner_id

        return {
            'initial_balances': self._get_initial_balance_values(report, partner_ids_to_expand, options),
            'aml_results': aml_results_per_partner_id,
            'has_more': has_more_per_partner_id,
        }
    ...
    def _query_values(self, report, options):
        ...
        :return:    [(record, values_by_column_group), ...],  where
                    - record is an res.partner record.
        ...
        for res in self._cr.dictfetchall():
            ...
            key = res['key']
            if key == 'sum':
                groupby_partners.setdefault(res['groupby'], {col_group_key: {} for col_group_key in options['column_groups']})
                groupby_partners[res['groupby']][column_group_key][key] = res
        ...
        # groupby_partners.keys() contains all partner ids affected by:
        # - the amls in the current period.
        # - the amls affecting the initial balance.
        # - the unaffected earnings allocation.
        # Note a search is done instead of a browse to preserve the table ordering.
        if groupby_partners:
            partners = self.env['res.partner'].search([('id', 'in', list(groupby_partners.keys()))])
        else:
            partners = []
    ...
    def _get_query_sums(self, report, options):
        ...
        for column_group_key, options_group in options_by_column_group.items():
            ...
            # Sum is computed including the initial balance of the partners configured to do so, unless a special option key is used
            ...
            if options.get('export_mode') == 'print' and options.get('filter_search_bar'):
                query_domain.append(('partner_id', 'ilike', options['filter_search_bar']))
    ...
    def _get_aml_values(self, report, options, expanded_partner_ids, offset=0, limit=None):
        rslt = {partner_id: {} for partner_id in expanded_partner_ids}
        aml_query, aml_params = self._get_query_amls(report, options, expanded_partner_ids, offset=offset, limit=limit)
        ...
        for aml_result in self._cr.dictfetchall():
            ...
            partner_result = rslt[aml_result['partner_id']]
            if not aml_key in partner_result:
                partner_result[aml_key] = {col_group_key: {} for col_group_key in options['column_groups']}

            already_present_result = partner_result[aml_key][aml_result['column_group_key']]
            if already_present_result:
                ...
                partner_result[aml_key][aml_result['column_group_key']] = aml_result

        return rslt, has_more

    def _get_query_amls(self, report, options, expanded_partner_ids, offset=0, limit=None):
        """ Construct a query retrieving the account.move.lines when expanding a report line with or without the load
        more.
        :param options:               The report options.
        :param expanded_partner_ids:  The res.partner ids corresponding to consider. If None, match every partner.
        :param offset:                The offset of the query (used by the load more).
        :param limit:                 The limit of the query (used by the load more).
        :return:                      (query, params)
        """
        additional_domain = [('partner_id', 'in', expanded_partner_ids)] if expanded_partner_ids is not None else None
        queries = []
        all_params = []
        lang = self.env.user.lang or get_lang(self.env).code
        journal_name = f"COALESCE(journal.name->>'{lang}', journal.name->>'en_US')" if \
            self.pool['account.journal'].name.translate else 'journal.name'
        # partner_name = f"partner_id.name"
    ...
    def _get_initial_balance_values(self, report, partner_ids, options):
        ...
        return {
            partner.id: (partner, init_balance_by_col_group[partner.id])
            for partner in partners
        }

    def _get_options_initial_balance(self, options):
        """ Create options used to compute the initial balances.
        The initial balances depict the current balance of the partners at the beginning of
    ...
    # COLUMN/LINE HELPERS
    ####################################################
    def _get_partner_title_line(self, report, options, partner, has_lines, eval_dict):
        line_columns = []
        for column in options['columns']:
            col_value = eval_dict.get(column['column_group_key'], {}).get(column['expression_label'])
            col_expr_label = column['expression_label']

            value = None if col_value is None else col_value
        ...
        is_in_unfolded_lines = any(
            report._get_res_id_from_line_id(line_id, 'res.partner') == partner.id
