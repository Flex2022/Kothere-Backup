...
class HRAttendance(models.Model):
    ...
    def _compute_attendance_work_hours(self):
        for rec in self:
            if rec.resource_calendar_id and rec.check_in and rec.check_out:
                hours = rec.employee_id._get_work_days_data_batch(rec.check_in, rec.check_out,
    ...
    def _compute_late_early_hours(self):
        ...
        for rec in self:
            calendar = rec.resource_calendar_id
            tz_employee = rec.employee_id.tz
            tz_calendar = rec.resource_calendar_id.tz
            if calendar and rec.check_in and rec.check_out:
                tz = tz_employee or tz_calendar
                ...
                for day in days_from_hours:
                    ...
                    check_out = user_timezone.localize(rec.check_out.replace(hour=out_hour, minute=out_min)).astimezone(
                        timezone('UTC'))
                    ...
                    work_day_start = user_timezone.localize(
                        rec.check_in.replace(hour=in_hour, minute=in_min)).astimezone(timezone('UTC')) - timedelta(
                        seconds=2)
                    ...
                    leaves = rec.employee_id._get_leave_days_data_batch(leaves_work_day_start, local_date,
                                                                        calendar=calendar)
                    if day.hour_from < hours_converted and day.hour_to > hours_converted and matched_recs and rec.id == \
                            matched_recs.ids[0]:
                        late_mins = hours_converted - day.hour_from
                        grace_period = day.grace_period
                        if prev_shift:
                            late_mins = 0
                    elif day.hour_from < hours_converted and day.hour_to > hours_converted and matched_recs1 and rec.id == \
                            matched_recs1.ids[0]:
                        late_mins = hours_converted - day.hour_from
                        grace_period = day.grace_period
                        if prev_shift:
                ...
                local_date_to = local_timezone.localize(rec.check_out).astimezone(timezone(self.env.user.tz if isinstance(self.env.user.tz, str) else 'UTC'))
                ...
                for day in days_to_hours:
                    ...
                    check_out = user_timezone.localize(
                        rec.check_out.replace(hour=out_hour1, minute=out_min)).astimezone(timezone('UTC'))
                    ...
                    if not att_after_checkout:
                        ...
                        leaves = rec.employee_id._get_leave_days_data_batch(local_date_to, work_day_end,
                                                                            calendar=calendar)
                        leave = leaves.get(rec.employee_id.id)
                        if hours_to_converted < day.hour_to and hours_to_converted > day.hour_from and (
                                (matched_rec and rec.id == matched_rec.ids[-1]) or shift_records and rec.id ==
                                shift_records.ids[0]):
                            early_checkout = day.hour_to - hours_to_converted
                            if leave.get('hours', False) and leave.get('hours') > 0:
                                early_checkout = day.hour_to - hours_to_converted - round(leave.get('hours'), 2) if (
                                                                                                                            day.hour_to - hours_to_converted - leave.get(
                                                                                                                        'hours')) > 0 else 0
                        elif hours_to_converted < day.hour_to and hours_to_converted >= day.hour_from and (
                                (matched_rec and rec.id == matched_rec.ids[-1]) or shift_records and rec.id ==
                                shift_records.ids[0]):
                            early_checkout = day.hour_to - hours_to_converted
                            flag = False
                            if leave.get('hours', False) and leave.get('hours') > 0:
                                early_checkout = day.hour_to - hours_to_converted - round(leave.get('hours'), 2) if (
                                                                                                                            day.hour_to - hours_to_converted - leave.get(
                                                                                                                        'hours')) > 0 else 0
                        elif hours_to_converted < day.hour_to and hours_to_converted > day.hour_from and matched_early_recs and rec.id == \
                                matched_early_recs.ids[-1] and not flag:
                            early_checkout = day.hour_to - hours_to_converted
                            if leave.get('hours', False) and leave.get('hours') > 0:
                                early_checkout = day.hour_to - hours_to_converted - round(leave.get('hours'), 2) if (
                                                                                                                            day.hour_to - hours_to_converted - leave.get(
