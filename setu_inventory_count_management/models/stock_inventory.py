...
class StockInventoryLine(models.Model):
    ...
    expired_inventory_id = fields.Many2one(
        comodel_name="setu.stock.inventory",
        string="Inventory"
    )
    damaged_inventory_id = fields.Many2one(
        comodel_name="setu.stock.inventory",
        string="Inventory"
    )
    ...
    expired_or_damaged_line = fields.Boolean(string="Expired/Damaged Line")
...
class StockInventory(models.Model):
    ...
    expired_line_ids = fields.One2many(
        'setu.stock.inventory.line', 'expired_inventory_id', string='Expired Inventories',
        copy=True,
        readonly=False
    )
    damaged_line_ids = fields.One2many(
        'setu.stock.inventory.line', 'damaged_inventory_id', string='Damaged Inventories',
        copy=True,
        readonly=False
    )
    ...
    def action_validate(self):
        ...
        for line in self.damaged_line_ids + self.expired_line_ids + self.line_ids:
            dont_do = False
            if line.product_id.tracking == 'serial':
                ...
                for sr_num in line.serial_number_ids:
                    quant = self.env['stock.quant'].sudo().search(
                        [('location_id', '=', line.location_id.id), ('lot_id', '=', sr_num.id),
                         ...
                         ('quantity', '>', 0)], limit=1)
                    if quant:
                        if line.expired_or_damaged_line:
                            dont_do = True
            ...
            if quant:
                line.quant_id = quant
                if auto_inventory_adjustment:
                    if not line.expired_or_damaged_line or not dont_do:
                    ...
                    if line.expired_or_damaged_line:
                        ...
                        moves = self.env['stock.move'].with_context(inventory_mode=False, adj_context=self.id).create(
                            move_vals)
                        move_time = moves.create_date
                        moves._action_done()
                        domain = [('location_id', '=', line.location_id.id),
                                  ('product_id', '=', line.product_id.id),
                                  ('create_date', '>=', move_time),
                                  ('quantity', '=', -1 * line.product_qty)]
                        if line.product_id.tracking != 'none':
                            domain.append(('lot_id', '=', quant.lot_id.id))
                        negative_quant = self.env['stock.quant'].sudo().search(domain, limit=1)
                        negative_quant and negative_quant.with_context(inventory_mode=True).write({'inventory_quantity': 0})
                        negative_quant.action_apply_inventory()
