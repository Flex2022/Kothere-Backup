...
class InventoryCountSessionLine(models.Model):
    ...
    not_found_serial_number_ids = fields.Many2many('stock.lot', 'session_not_found_stock_lot_rel', 'session_line_id',
                                                   'lot_id', string="Missing Serials")
    is_system_generated = fields.Boolean(
        string="System Line?"
    )
    unscanned_product_line_id = fields.Many2one('setu.unscanned.product.lines', string="Unscanned")
    expired_serial_number_ids = fields.Many2many('stock.lot', 'expired_serial_rel', 'session_line_id', 'expired_lot_id',
                                                 string="Expired Serials")
    computed_expired_serial_number_ids = fields.Many2many('stock.lot',
                                                          compute="compute_expired_serial_numbers",
                                                          string="expired_serial_numbers")
    damaged_serial_number_ids = fields.Many2many('stock.lot', 'damaged_serial_rel', 'session_line_id', 'damaged_lot_id',
                                                 string="Damaged Serials")
    counted_expired_qty = fields.Float(string="Expired")
    counted_damaged_qty = fields.Float(string="Damaged")
    is_expired_lot = fields.Boolean(string="Is Expired Lot", compute="compute_is_expired_lot")

    @api.depends('lot_id')
    def compute_is_expired_lot(self):
        for record in self:
            record.is_expired_lot = True
            if record.product_id.tracking == 'lot':
                record.is_expired_lot = True if (
                        record.product_id.use_expiration_date and record.lot_id.expiration_date and
                        record.lot_id.expiration_date <= fields.Datetime.now()) else False
            # elif record.product_id.tracking == 'serial':
            #     record.is_expired_lot = True

    @api.depends('product_id')
    def compute_expired_serial_numbers(self):
        for record in self:
            record.computed_expired_serial_number_ids = False
            if (record.product_id and self.env.user.has_group(
                    "setu_inventory_count_management.group_include_expiry_counting") and record.tracking == 'serial'
                    and record.product_id.use_expiration_date):
                current_date = fields.Datetime.now()
                record.computed_expired_serial_number_ids = self.env['stock.lot'].search(
                    [('product_id', '=', record.product_id.id), ('expiration_date', '<=', current_date)])
    ...
    @api.constrains('scanned_qty', 'counted_expired_qty', 'counted_damaged_qty')
    def constrains_scanned_qty(self):
        for line in self:
            if line.scanned_qty < 0 or line.counted_expired_qty < 0 or line.counted_damaged_qty < 0:
                raise UserError('Counted or Expired or Damaged Quantity cannot be less than zero.')

    @api.constrains('serial_number_ids', 'expired_serial_number_ids', 'damaged_serial_number_ids')
    def constrains_scanned_serial_numbers(self):
        duplicates = []
        for line in self.session_id.session_line_ids:
            if ((line.serial_number_ids.product_id != line.product_id if line.serial_number_ids else False)
                    or (
                            line.damaged_serial_number_ids.product_id != line.product_id if line.damaged_serial_number_ids else False)
                    or (
                            line.expired_serial_number_ids.product_id != line.product_id if line.expired_serial_number_ids else False)):
                raise UserError(f'Some Serial Numbers Does Not Belongs To {line.product_id.name}')
            serial_numbers = line.damaged_serial_number_ids.ids + line.serial_number_ids.ids + line.expired_serial_number_ids.ids
            seen = set()
            duplicates.append([x for x in serial_numbers if x in seen or seen.add(x)])
        duplicates = [element for nestedlist in duplicates for element in nestedlist]
        if duplicates:
            same_found = self.env['stock.lot'].sudo().browse(duplicates)
            same_found_str = ", ".join(same_found.mapped('name'))
            raise UserError(f'Serial Number "{same_found_str}" is already scanned in this session')

    @api.constrains('lot_id')
    def constrains_scanned_aty(self):
        for line in self.session_id.session_line_ids:
            if line.lot_id and line.lot_id.product_id != line.product_id:
                raise UserError(f'{line.lot_id.name} Does Not Belongs To {line.product_id.name}')
    ...
    @api.depends('lot_id', 'product_id', 'serial_number_ids', 'expired_serial_number_ids',
                 'damaged_serial_number_ids', 'location_id')
    ...
    @api.onchange('product_id', 'lot_id', 'location_id', 'expired_serial_number_ids', 'serial_number_ids',
                  'damaged_serial_number_ids')
    def _onchange_product_id(self):
        if self._origin.product_id and self.product_id.id != self._origin.product_id.id:
            self.lot_id = False
            self.serial_number_ids = False
            self.expired_serial_number_ids = False
            self.damaged_serial_number_ids = False
        if self.location_id:
        ...
        if not self.session_id.session_id and not self.session_id.inventory_count_id.count_id:
            ...
            if self.product_id.tracking == 'lot' and self.product_id and self.location_id and self.lot_id and not self.session_id.session_id:
                ...
                if self.product_id.id in session_lines.filtered(lambda x: x.product_id == self.product_id
                    ...
                    or any(b in x.damaged_serial_number_ids.ids for b in self.damaged_serial_number_ids.ids)
                    or any(b in x.expired_serial_number_ids.ids for b in self.expired_serial_number_ids.ids))
                                                                          # and any() in
                                                                          and x.session_id.inventory_count_id == self.session_id.inventory_count_id).mapped(
                    'product_id').ids:
        ...
        if self.product_id.tracking == 'serial':
            ...
            self.counted_expired_qty = len(self.expired_serial_number_ids)
            self.counted_damaged_qty = len(self.damaged_serial_number_ids)
        if self.product_id.tracking != 'serial':
            self.scanned_qty = 0
            self.counted_expired_qty = 0
            self.counted_damaged_qty = 0

    @api.onchange('lot_id', 'product_id', 'serial_number_ids', 'expired_serial_number_ids',
                  'damaged_serial_number_ids', 'location_id')
    ...
    def _get_counted_qty(self, line, count_line_exists_already=False):
        if line.product_id.tracking == 'serial':
            ...
            expired_serial_numbers = serial_type_session_lines.mapped('expired_serial_number_ids').filtered(
                lambda s: s.product_qty < 1)
            damaged_serial_numbers = serial_type_session_lines.mapped('damaged_serial_number_ids').filtered(
                lambda s: s.product_qty < 1)
            final_serial_numbers = serial_type_session_lines.mapped(
                'serial_number_ids') - serial_numbers - line.damaged_serial_number_ids
            expired_final_serial_numbers = serial_type_session_lines.mapped(
                'expired_serial_number_ids') - expired_serial_numbers
            damaged_final_serial_numbers = serial_type_session_lines.mapped(
                'damaged_serial_number_ids') - damaged_serial_numbers - line.serial_number_ids
            return len(final_serial_numbers), len(expired_final_serial_numbers), len(damaged_final_serial_numbers)
        elif line.product_id.tracking in ('none', 'lot'):
            ...
            return counted_qty, counted_damaged_qty, counted_expired_qty
    ...
    def _get_serial_number_ids(self, line):
        ...
        damaged_final_serial_numbers = serial_type_session_lines.mapped(
            'damaged_serial_number_ids') - damaged_serial_numbers - line.serial_number_ids
        return [(6, 0, final_serial_numbers.ids)], [(6, 0, damaged_final_serial_numbers.ids)], [
            (6, 0, expired_final_serial_numbers.ids)]
