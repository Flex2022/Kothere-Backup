...
class StockInvCount(models.Model):
    ...
    def write(self, vals):
        ...
        for record in self:
            sessions = record.session_ids.filtered(lambda x: x.state in ['Draft', 'In Progress'])
            sessions.write({'use_barcode_scanner': record.use_barcode_scanner})
        return res
    ...
    def create_inventory_adj(self):
        if self.type == 'Single Session':
            lines_to_adjust = self.line_ids.filtered(lambda l: l.is_discrepancy_found or l.expired_or_damaged_line)
        else:
            lines_to_adjust = self.line_ids.filtered(
                lambda l: (l.is_discrepancy_found or l.expired_or_damaged_line) and l.state == 'Approve')
        if lines_to_adjust:
        ...
        if count_lines:
            ...
            for l in count_lines:
                if l.product_id.tracking != 'serial':
                    if l.counted_expired_qty:
                        location_id = self.env['inventory.count.settings'].search([]).expiry_location_id.id
                        expired_lines.append((
                            0, 0, {'product_id': l.product_id.id, 'product_uom_id': l.product_id.uom_id.id,
                                   'location_id': l.location_id.id, 'location_dest_id': location_id,
                                   'product_qty': l.counted_expired_qty,
                                   'prod_lot_id': l.lot_id.id if l.lot_id else False,
                                   'theoretical_qty': l.qty_in_stock, 'expired_or_damaged_line': True}))
                    if l.counted_damaged_qty:
                        location_id = self.env['inventory.count.settings'].search([]).damage_location_id.id
                        damaged_lines.append((
                            0, 0, {'product_id': l.product_id.id, 'product_uom_id': l.product_id.uom_id.id,
                                   'location_id': l.location_id.id, 'location_dest_id': location_id,
                                   'product_qty': l.counted_damaged_qty,
                                   'prod_lot_id': l.lot_id.id if l.lot_id else False,
                                   'theoretical_qty': l.qty_in_stock, 'expired_or_damaged_line': True}))
                    if not l.expired_or_damaged_line or (l.counted_qty - l.theoretical_qty != 0):
                        lines.append((
                            0, 0, {'product_id': l.product_id.id, 'product_uom_id': l.product_id.uom_id.id,
                    ...
                    if l.serial_number_ids or l.expired_serial_number_ids or l.damaged_serial_number_ids:
                        ...
                        for damage_serial in l.damaged_serial_number_ids:
                            location_id = self.env['inventory.count.settings'].search([]).damage_location_id.id
                            damaged_lines.append((
                                0, 0, {'product_id': l.product_id.id, 'product_uom_id': l.product_id.uom_id.id,
                                       'location_id': l.location_id.id, 'location_dest_id': location_id,
                                       'product_qty': 1,
                                       'prod_lot_id': l.lot_id.id if l.lot_id else False,
                                       'serial_number_ids': [(6, 0, damage_serial.ids)],
                                       'theoretical_qty': l.qty_in_stock,
                                       'expired_or_damaged_line': True}))
                        for expired_serial in l.expired_serial_number_ids:
                            location_id = self.env['inventory.count.settings'].search([]).expiry_location_id.id
                            expired_lines.append((
                                0, 0, {'product_id': l.product_id.id, 'product_uom_id': l.product_id.uom_id.id,
                                       'location_id': l.location_id.id, 'location_dest_id': location_id,
                                       'product_qty': 1,
                                       'prod_lot_id': l.lot_id.id if l.lot_id else False,
                                       'serial_number_ids': [(6, 0, expired_serial.ids)],
                                       'theoretical_qty': l.qty_in_stock,
                                       'expired_or_damaged_line': True}))
                        ...
                        for m in l.not_found_serial_number_ids:
                            lot_exists = self.env['stock.quant'].sudo().search(
                                [('location_id', '=', l.location_id.id),
                                 ...
                                 ('quantity', '>', 0),
                                 ('product_id', '=', l.product_id.id)]).mapped('lot_id')
                            # if m.expiration_date and m.expiration_date <= fields.Datetime.now():
                            #     location_id = self.env['inventory.count.settings'].search([]).expiry_location_id.id
                            #     expired_lines.append((
                            #         0, 0, {'product_id': l.product_id.id, 'product_uom_id': l.product_id.uom_id.id,
                            #                'location_id': l.location_id.id, 'location_dest_id': location_id,
            ...
            adj = self.env['setu.stock.inventory'].create({
                ...
                'line_ids': lines,
                'expired_line_ids': expired_lines,
                'damaged_line_ids': damaged_lines
            })
    ...
    def check_unscanned(self):
        ...
        not_found_serial_numbers = quants.filtered(lambda
                                                       x: x.product_id.id in self.line_ids.product_id.ids and x.lot_id and x.lot_id.id not in self.line_ids.lot_id.ids + self.line_ids.serial_number_ids.ids + self.line_ids.expired_serial_number_ids.ids + self.line_ids.damaged_serial_number_ids.ids)
    ...
    def count_data_according_to_warehouse(self):
        auto_inventory = self.env['inventory.count.settings'].search([])
        end_date = date.today()
        start_date = date.today() - timedelta(days=auto_inventory.inventory_count_days)
        query = """Select * from get_sales_transaction_data('%s','%s','%s','%s','%s','%s')""" \
                % ({}, {}, {}, {}, start_date, end_date)
        self._cr.execute(query)
        count_data = self._cr.dictfetchall()
        data = {}
        for count in count_data:
            warehouse = count['warehouse_id']
            record = {'product_id': count['product_id'],
                      'location_id': self.env['stock.warehouse'].browse(count['warehouse_id']).lot_stock_id.id, }
            if warehouse in data:
                data[warehouse].append(record)
            else:
                data[warehouse] = [record]
        return data, auto_inventory

    def auto_create_inventory_count_as_per_configuration(self, data={}, auto_inventory=False, users=False):
        if not data and not auto_inventory:
            data, auto_inventory = self.count_data_according_to_warehouse()
        if not users:
            users = auto_inventory.session_user_ids.ids
        count_ids = []
        for rec in data:
            inventory_count = self.sudo().create({'approver_id': auto_inventory.approver_id.id,
                                                  'use_barcode_scanner': auto_inventory.use_barcode_scanner,
                                                  'type': 'Multi Session',
                                                  'warehouse_id': rec,
                                                  'location_id': self.env['stock.warehouse'].browse(
                                                      rec).lot_stock_id.id,
                                                  })
            inv_session = inventory_count.sudo().create_session()
            wizard = self.env['inventory.session.creator'].sudo().browse(inv_session['res_id'])
            wizard.sudo().write({'user_ids': [(6, 0, users)]})
            wizard.sudo().confirm()
            inventory_count.sudo().session_ids.sudo().write({'session_line_ids': [(0, 0, line) for line in data[rec]]})
            inventory_count.session_ids.session_line_ids._get_theoretical_qty()
            count_ids.append(inventory_count.id)
        if self._context.get('from_pos_session', False):
            return count_ids
        return True
