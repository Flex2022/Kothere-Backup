...
class InventoryCountSession(models.Model):
    ...
    # For Expiry and Damage Functionality
    scanning_for = fields.Selection(string='Scanning For',
                                    required=True, default='normal', selection="_get_scanning_for")
    warning_text = fields.Text(string="Warning")

    # @api.model
    # def get_views(self):
    #     super(InventoryCountSession, self).get_views()

    @api.onchange('scanning_for')
    def onchange_scanning_for(self):
        self.current_scanning_product_id = False
        self.current_scanning_location_id = False
        self.current_scanning_lot_id = False

    def _get_scanning_for(self):
        vals = [('normal', 'Normal')]
        if self.env.user.has_group('setu_inventory_count_management.group_include_expiry_counting'):
            vals.append(('expiry', 'Expiry'))
        if self.env.user.has_group('setu_inventory_count_management.group_include_damage_counting'):
            vals.append(('damage', 'Damage'))
        return vals
    ...
    def check_session_line_ids(self):
        ...
        session_lines = self.session_line_ids.filtered(lambda
                                                           x: x.tracking == 'serial' and not x.serial_number_ids and not x.damaged_serial_number_ids and not x.expired_serial_number_ids)
        if session_lines:
            self.warning_text = 'Serial Numbers are not scanned for the product ' + ''.join(session_lines.product_id.mapped('name'))
        else:
            self.warning_text = ''
    ...
    def on_barcode_scanned(self, barcode):
        if self.use_barcode_scanner:
            if self.current_state in ('Start', 'Resume'):
                if barcode == 'normal':
                    self.scanning_for = 'normal'
                elif barcode == 'expiry':
                    self.scanning_for = 'expiry'
                elif barcode == 'damage':
                    self.scanning_for = 'damage'
                else:
                    vals = {}
                    scanning_done = False
                    ...
                    if lot:
                        ...
                        if not self.session_id and not self.session_id.inventory_count_id.count_id:
                            session_lines = self.inventory_count_id.session_ids.session_line_ids + self.session_line_ids
                            if self.current_scanning_product_id.tracking == 'serial' and self.current_scanning_product_id and self.current_scanning_location_id:
                                if self.current_scanning_product_id.id in session_lines.filtered(
                                        lambda x: x.product_id == self.current_scanning_product_id and
                                                  x.location_id != self.current_scanning_location_id
                                                  and (lot.id in x.serial_number_ids.ids or
                                                       lot.id in x.expired_serial_number_ids.ids or
                                                       lot.id in x.damaged_serial_number_ids.ids)
                                                  and x.session_id.state != 'Cancel'
                                                  and x.inventory_count_id == self.inventory_count_id).mapped(
                                    'product_id').ids:
                                    # and x.session_id.id != self._origin.id and x.location_id == self.current_scanning_location_id
                                    raise UserError(
                                        'Serial Number "{}" is already scanned in another location or in expired/damaged serial numbers by another user in '
                    ...
                    if self.scanning_for != 'expiry' and self.env.user.has_group(
                            'setu_inventory_count_management.group_include_expiry_counting'):
                        if lot.product_id.use_expiration_date and lot.expiration_date and lot.expiration_date <= fields.Datetime.now():
                            return self.messege_return("Warning",
                                                       f"{lot.name} is already expired.")

                    if self.scanning_for == 'expiry' and self.env.user.has_group(
                            'setu_inventory_count_management.group_include_expiry_counting'):
                        if lot.product_id.use_expiration_date and lot.expiration_date and not lot.expiration_date <= fields.Datetime.now():
                            return self.messege_return("Warning",
                                                       f"{lot.name} is not expired.")
                    ...
                    if self.current_scanning_product_id.tracking == 'serial':
                        ...
                        if self.scanning_for == 'expiry':
                            vals.update({'expired_serial_number_ids': [(4, lot.id)], 'theoretical_qty': qty_available})
                        elif self.scanning_for == 'damage':
                            vals.update({'damaged_serial_number_ids': [(4, lot.id)], 'theoretical_qty': qty_available})
                        else:
                        ...
                        if product.tracking == 'none':
                            ...
                            if self.current_scanning_product_id.tracking == 'none' and self.current_scanning_product_id and self.current_scanning_location_id:
                                if self.current_scanning_product_id.id in session_lines.filtered(
                                        lambda x: x.product_id == self.current_scanning_product_id
                                                  and x.session_id.state != 'Cancel'
                                                  and x.location_id == self.current_scanning_location_id and x.inventory_count_id == self.inventory_count_id and
                                                  x.session_id.id == self._origin.id).mapped('product_id').ids:
                            ...
                            none_type_product_line_already_exist = self.session_line_ids.filtered(
                                lambda
                                    x: x.location_id == self.current_scanning_location_id and x.product_id == product)
                            if not none_type_product_line_already_exist:
                                ...
                                if self.scanning_for == 'expiry':
                                    session_line.update({'counted_expired_qty': 1})
                                elif self.scanning_for == 'damage':
                                    session_line.update({'counted_damaged_qty': 1})
                                else:
                                    session_line.update({'scanned_qty': 1})
                                self.write({'session_line_ids': [(0, 0, session_line)]})
                                # self.env['setu.inventory.count.session.line'].create()
                            else:
                                if self.scanning_for == 'expiry':
                                    none_type_product_line_already_exist.write(
                                        {
                                            'counted_expired_qty': none_type_product_line_already_exist.counted_expired_qty + 1})
                                elif self.scanning_for == 'damage':
                                    none_type_product_line_already_exist.write(
                                        {
                                            'counted_damaged_qty': none_type_product_line_already_exist.counted_damaged_qty + 1})
                                else:
                                    none_type_product_line_already_exist.write(
                                ...
                                if session_line:
                                    if session_line.product_scanned:
                                        return self.messege_return("Warning",
                                                                   ...
                                                                   f"Please uncheck the Scanned checkbox.")
                                    if session_line.is_expired_lot and (
                                            self.scanning_for == 'normal' or self.scanning_for == 'damage'):
                                        return self.messege_return("Warning",
                                                                   f"{session_line.lot_id.name} is already expired ")
                                    # For Expiry and Damage Functionality
                                    if self.scanning_for == 'normal' and session_line.lot_id:
                                        new_scan_qty = session_line.scanned_qty + 1
                                        session_line.write({'scanned_qty': new_scan_qty})
                                        scanning_done = True
                                    elif self.scanning_for == 'damage' and session_line.lot_id:
                                        new_scan_qty = session_line.counted_damaged_qty + 1
                                        session_line.write({'counted_damaged_qty': new_scan_qty})
                                        scanning_done = True
                                    elif self.scanning_for == 'expiry' and session_line.lot_id:
                                        new_scan_qty = session_line.counted_damaged_qty + 1
                                        session_line.write({'counted_expired_qty': new_scan_qty})
                                        scanning_done = True
                    ...
                    if not scanning_done and self.current_scanning_location_id and self.current_scanning_product_id and self.current_scanning_lot_id:
                        ...
                        if self.current_scanning_product_id.tracking == 'lot':
                            line_already_exist = self.session_line_ids.filtered(
                                lambda
                                    x: x.location_id == self.current_scanning_location_id and x.lot_id == self.current_scanning_lot_id and x.product_id == self.current_scanning_product_id)
                        ...
                        if line_already_exist and self.current_scanning_product_id.tracking == 'serial':
                            if self.current_scanning_lot_id.id not in (
                                    line_already_exist.serial_number_ids.ids or line_already_exist.expired_serial_number_ids.ids or line_already_exist.damaged_serial_number_ids.ids):
                                if self.scanning_for == 'expiry':
                                    expiry_count = len(line_already_exist.expired_serial_number_ids) + 1
                                    line_already_exist.write(
                                        {'expired_serial_number_ids': [(4, self.current_scanning_lot_id.id)],
                                         'counted_expired_qty': expiry_count})
                                elif self.scanning_for == 'damage':
                                    damage_count = len(line_already_exist.damaged_serial_number_ids) + 1
                                    line_already_exist.write(
                                        {'damaged_serial_number_ids': [(4, self.current_scanning_lot_id.id)],
                                         'counted_damaged_qty': damage_count})
                                else:
                                    count = len(line_already_exist.serial_number_ids) + 1
                                    line_already_exist.write(
                                        {'serial_number_ids': [(4, self.current_scanning_lot_id.id)],
                                         'scanned_qty': count})
                        if not line_already_exist:
                            if self.current_scanning_product_id.tracking == 'lot':
                                if self.current_scanning_lot_id.product_id.use_expiration_date and self.current_scanning_lot_id.expiration_date <= fields.Datetime.now():
                                    vals.update({'counted_expired_qty': vals['theoretical_qty']})
                                vals.update({'lot_id': lot.id})
                            if self.current_scanning_product_id.tracking == 'serial':
                                if self.scanning_for == 'expiry':
                                    vals.update({'expired_serial_number_ids': [(4, lot.id)],
                                                 'counted_expired_qty': 1})
                                elif self.scanning_for == 'damage':
                                    vals.update({'damaged_serial_number_ids': [(4, lot.id)],
                                                 'counted_damaged_qty': 1})
                                else:
    ...
    def open_new_session(self):
        ...
        for line in rejected_lines:
            ...
            new_session_line.expired_serial_number_ids = False
            new_session_line.damaged_serial_number_ids = False
            new_session_line.scanned_qty = 0
            new_session_line.counted_expired_qty = 0
            new_session_line.counted_damaged_qty = 0
    ...
    def submit(self):
            ...
            if line.tracking == 'serial':
                same_found = session_lines.filtered(lambda x: x.product_id == line.product_id
                    ...
                    b in x.serial_number_ids.ids for b in line.serial_number_ids.ids)
                                                                   or any(
                            b in x.damaged_serial_number_ids.ids for b in line.serial_number_ids.ids if
                            x.session_id.id == line.session_id.id)
                                                                   or any(
                            b in x.serial_number_ids.ids for b in line.damaged_serial_number_ids.ids if
                            x.session_id.id == line.session_id.id)
                                                                   or any(
                            b in x.damaged_serial_number_ids.ids for b in line.damaged_serial_number_ids.ids)
                                                                   or any(
                            b in x.expired_serial_number_ids.ids for b in line.expired_serial_number_ids.ids))
                                                              and x.session_id.inventory_count_id == line.session_id.inventory_count_id)
                if same_found:
                    same_found = list(set(same_found.mapped('serial_number_ids').ids +
                                          same_found.mapped('damaged_serial_number_ids').ids +
                                          same_found.mapped('expired_serial_number_ids').ids)
                                      & set(line.serial_number_ids.ids + line.expired_serial_number_ids.ids +
                                            line.damaged_serial_number_ids.ids))
                ...
                if count_line_exists_already:
                    counted_qty, counted_damaged_qty, counted_expired_qty = line._get_counted_qty(line,
                                                                                                  count_line_exists_already)
                    serial_numbers, damaged_serial_numbers, expired_serial_numbers = line._get_serial_number_ids(line)
                    count_line_exists_already.write(
                        {'serial_number_ids': [
                            (6, 0, line.serial_number_ids.ids)] if self.session_id else serial_numbers,
                         'damaged_serial_number_ids': [
                             (6, 0, line.damaged_serial_number_ids.ids)] if self.session_id else damaged_serial_numbers,
                         'expired_serial_number_ids': [
                             (6, 0, line.expired_serial_number_ids.ids)] if self.session_id else expired_serial_numbers,
                         ...
                         'counted_damaged_qty': counted_damaged_qty,
                         'counted_expired_qty': counted_expired_qty,
                         'expired_or_damaged_line': True if counted_damaged_qty or counted_expired_qty else False
                         })
                else:
                    new_line = self.create_new_count_line(line)

                if new_line:
                    found_lots = (new_line.serial_number_ids + new_line.expired_serial_number_ids +
                                  new_line.damaged_serial_number_ids + count_line_exists_already.serial_number_ids +
                                  count_line_exists_already.expired_serial_number_ids +
                                  count_line_exists_already.damaged_serial_number_ids)
                else:
                ...
                if count_line_exists_already:
                    ...
                    #     counted_damaged_qty = line.counted_damaged_qty
                    #     counted_expired_qty = line.counted_expired_qty
                    # else:
                    #     counted_qty = count_line_exists_already.counted_qty + line.scanned_qty
                    #     counted_damaged_qty = count_line_exists_already.counted_damaged_qty + line.counted_damaged_qty
                    #     counted_expired_qty = count_line_exists_already.counted_expired_qty + line.counted_expired_qty
                    count_line_exists_already.write(
                        {'theoretical_qty': line._get_theoretical_qty(count_line_exists_already),
                         'qty_in_stock': line._get_theoretical_qty(count_line_exists_already),
                         ...
                         'counted_damaged_qty': counted_damaged_qty,
                         'counted_expired_qty': counted_expired_qty,
                         'lot_id': line.lot_id.id or count_line_exists_already.lot_id.id,
                         'expired_or_damaged_line': True if counted_damaged_qty or counted_expired_qty
                                                            or line.expired_serial_number_ids or line.damaged_serial_number_ids else False
                         })
                else:
                    self.create_new_count_line(line)
        ...
        if not_found_serial:
            ...
            added_serial_number_ids = count_line.serial_number_ids + count_line.expired_serial_number_ids + count_line.damaged_serial_number_ids
    ...
    def _create_new_line(self, line):
        new_line = self.env['setu.stock.inventory.count.line'].create({
            ...
            'expired_serial_number_ids': [
                (6, 0, line.expired_serial_number_ids.ids)] if line.expired_serial_number_ids else False,
            'damaged_serial_number_ids': [
                (6, 0, line.damaged_serial_number_ids.ids)] if line.damaged_serial_number_ids else False,
            ...
            'counted_qty': 0,
            'user_calculation_mistake': line.user_calculation_mistake,
            'expired_or_damaged_line': True if line.counted_damaged_qty or line.counted_expired_qty
                                               or line.expired_serial_number_ids or line.damaged_serial_number_ids else False
        })
        return new_line
    ...
    def create_new_count_line(self, line):
        ...
        expired_qty = line.counted_expired_qty
        damaged_qty = line.counted_damaged_qty
        # if line.tracking == 'lot':
        #     moves = self.env['stock.move.line'].sudo().search([('state', '=', 'done'),
        #                                                        ('product_id', '=', line.product_id.id),
        #                                                        ('move_id.picking_type_id.code', '=', 'outgoing'),
        #                                                        '|', '|', ('lot_id', '=', line.lot_id.id),
        #                                                        ('lot_id', '=', line.expired_lot_id.id),
        #                                                        ('lot_id', '=', line.damaged_lot_id.id),
        ...
        #                                                        ('lot_id.id', 'in', line.expired_serial_number_ids.ids),
        #                                                        ('lot_id.id', 'in', line.damaged_serial_number_ids.ids),
        #                                                        ('date', '>=', line.date_of_scanning)])
        # moves = self.env['stock.move.line'].search([]).filtered(
        #     lambda
        ...
        # if qty + damaged_qty > sum([x.qty_done for x in moves]):
        #     qty -= sum([x.qty_done for x in moves])
        # elif expired_qty > sum([x.qty_done for x in moves]):
        #     expired_qty -= sum([x.qty_done for x in moves])
        # elif damaged_qty > sum([x.qty_done for x in moves]):
        #     damaged_qty -= sum([x.qty_done for x in moves])
        # else:
        #     qty += 0
        #     expired_qty += 0
        #     damaged_qty += 0
        ...
        new_count_line.write({
            ...
            'counted_qty': qty,
            'counted_expired_qty': expired_qty,
            'counted_damaged_qty': damaged_qty
        })
    ...
    def _set_calculation_mistake_value(self, line, parent, count_line, real_quantity=False):
        if type(real_quantity) == bool and not real_quantity:
            real_quantity = line.scanned_qty + line.counted_expired_qty + line.counted_damaged_qty
        if parent:
            ...
            if tracking == 'lot':
                ...
                parent_session_line = parent.session_line_ids.filtered(
                    lambda l: l.product_id == line.product_id and l.location_id == line.location_id
                              and any(
                        b in l.serial_number_ids.ids or b in l.expired_serial_number_ids.ids or b in l.damaged_serial_number_ids.ids
                        for b in line.serial_number_ids.ids))
            else:
                parent_session_line = parent.session_line_ids.filtered(
                    lambda l: l.product_id == line.product_id and l.location_id == line.location_id)
            ...
            if parent_session_line:
                if real_quantity != parent_session_line.scanned_qty + parent_session_line.counted_expired_qty + parent_session_line.counted_damaged_qty:
            ...
            if 0 <= real_quantity != line.scanned_qty + line.counted_expired_qty + line.counted_damaged_qty:
    ...
    def _validate_session(self):
        ...
        for line in session_lines:
            ...
            if count_line and line.is_system_generated and self.type == 'Single Session':
                ...
                count_line.counted_expired_qty = line.counted_expired_qty
                count_line.counted_damaged_qty = line.counted_damaged_qty
