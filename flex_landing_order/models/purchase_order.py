...
class PurchaseOrder(models.Model):
    ...
    def _compute_landing_orders_count(self):
        for order in self:
            order.landing_orders_count = self.env['landing.order'].search_count([('purchase_order_id', '=', order.id)])

    @api.onchange('order_type')
    def _onchange_order_type(self):
        if self.landing_orders_count > 0:
            raise UserError(
                _('You can not change the order type to landing order because there is landing orders related to this purchase order'))