...
class JobEstimate(models.Model):
    ...
    def _calculate_total(self):
        ...
        for order in self:
            ...
            if order.material_estimation_ids_other:
                for line in order.material_estimation_ids_other:
                    other_price = (line.quantity * line.unit_price) - (line.quantity * line.unit_price) * (
                            line.discount or 0.0) / 100.0
                    order.total_other_estimate += other_price
                    total_job_cost += other_price
            else:
                order.total_other_estimate = 0
    ...
    material_estimation_ids_other = fields.One2many('other.estimate', 'other_id', 'Other Estimation')
    ...
    total_job_estimate = fields.Float(compute='_calculate_total', string='Total Job Estimate', default=0.0,
                                      readonly=True)
    total_other_estimate = fields.Float(compute='_calculate_total', string='Total Other Estimate', default=0.0,
                                        readonly=True)
...
class MaterialEstimate(models.Model):
    ...
    def _compute_unit_price(self):
        for product in self:
            product.unit_price = product.cost + (product.cost * (product.margin / 100))
...
class LabourEstimate(models.Model):
    ...
    type = fields.Selection([('labour', 'Processing')], string='Type', default='labour', readonly=True)
    product_id = fields.Many2one('product.product', 'Product', required=True)
    description = fields.Text('Description')
    quantity = fields.Float('Quantity', default=0.0)
    uom_id = fields.Many2one('uom.uom', 'Unit Of Measure')
...
class OverheadEstimate(models.Model):
    ...
    type = fields.Selection([('overhead', 'Misc')], string='Type', default='overhead', readonly=True)
    product_id = fields.Many2one('product.product', 'Product', required=True)
    description = fields.Text('Description')
    quantity = fields.Float('Quantity', default=0.0)
    uom_id = fields.Many2one('uom.uom', 'Unit Of Measure')
...
class OtherEstimate(models.Model):
    _name = "other.estimate"
    _description = 'Create Overhead Estimate'
    ...
    other_id = fields.Many2one('job.estimate', 'other')
    type = fields.Selection([('other', 'Other')], string='Type', default='other', readonly=True)
    product_id = fields.Many2one('product.product', 'Product', required=True)
    description = fields.Text('Description')
    quantity = fields.Float('Quantity', default=0.0)
    uom_id = fields.Many2one('uom.uom', 'Unit Of Measure')
...
class SaleOrder(models.Model):
    ...
    job_cost_sheet_id = fields.Many2one(comodel_name='job.cost.sheet', string='Job Cost Sheet',
                                        compute='_job_cost_sheet_id', store=True)
