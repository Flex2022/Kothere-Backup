...
class HrPayroll(models.Model):
    ...
    pump_workers_no = fields.Float(string='Quantity Workers', compute='_compute_pump_workers_no')
    operator_quantity = fields.Float(string='Operator Quantity', compute='_compute_operator_quantity')
    ...
    def _compute_pump_workers_no(self):
        for record in self:
            if record.employee_id:
                ...
                # sum pump_no of all delivery notes
                pump_no = sum(delivery_notes.mapped('quantity'))
    ...
    @api.depends('employee_id', 'date_from', 'date_to')
    def _compute_operator_quantity(self):
        for record in self:
            if record.employee_id:
                delivery_notes = self.env['delivery.note'].search([
                    ('operator_ids', 'in', [record.employee_id.id]),  # Wrap the single ID in a list
                    ('delivery_date_without_time', '>=', record.date_from),
                    ('delivery_date_without_time', '<=', record.date_to),
                    ('state', '=', 'done')
                ])
                ...
                # sum pump_no of all delivery notes
                pump_no = sum(delivery_notes.mapped('quantity'))
                record.operator_quantity = pump_no
            else:
                record.operator_quantity = 0
